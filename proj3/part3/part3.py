...
suite = unittest.TestLoader().loadTestsFromTestCase(TestAddressMethods)
...
class Protocol(object):
    ...
    PIM_TOKEN = 'PIM'
    ICMP_TOKEN = 'ICMP'
    VALID = [TCP_TOKEN, UDP_TOKEN, VRPP_TOKEN, IGMP_TOKEN, PIM_TOKEN, ICMP_TOKEN]
    ...
    def __init__(self, raw):
        ...
        self.value = int(raw[raw.find(Protocol.OPEN_PAREN) + 1:raw.find(Protocol.CLOSE_PAREN)])
...
suite = unittest.TestLoader().loadTestsFromTestCase(TestProtocolMethods)
unittest.TextTestRunner().run(suite)
    ...
    def __init__(self, data):
        ...
        for field in self.fields:
            if field[:len(DatagramHeader.TOS_TOKEN)] == DatagramHeader.TOS_TOKEN:
                ...
                self.protocol = Protocol(field[len(DatagramHeader.PROTOCOL_TOKEN) + 1:])
...
suite = unittest.TestLoader().loadTestsFromTestCase(TestDatagramHeader)
unittest.TextTestRunner().run(suite)
...
class IPPacket(object):
    ...
    def __init__(self, data):
        ...
        end_idx = data.rfind(IPPacket.CLOSE_PAREN, 0, data.find(IPPacket.COLON_TOKEN))
        ...
        src_dst = [t.strip() for t in data[end_idx+1:data.find(IPPacket.COLON_TOKEN)].split(" > ")]
        ...
        if self.header.protocol.name == 'TCP':
            info = [t.strip() for t in data[data.find(IPPacket.COLON_TOKEN) + 1:].split(IPPacket.SEP)]
            ...
            for item in info:
                if item[:len(IPPacket.FLAGS_TOKEN)] == IPPacket.FLAGS_TOKEN:
                    ...
                    CHECK(self.options, lambda:  self.options is None)
                    self.options = getList(item[len(IPPacket.OPTIONS_TOKEN) + 1:])
...
suite = unittest.TestLoader().loadTestsFromTestCase(TestIPPacketTest)
unittest.TextTestRunner().run(suite)
...
"IP packets: %s, ARP: %s, IP6: %s" % (len(packets), len(ARP), len(IP6))
...
def isMobilePacket(p):
    ...
    TCP_HANDSHAKE = ["S", "S.", "."]
    return (((p.src.ip == CELL_IP and p.dst.port in WEBSITE_HOST_PORT)
            or (p.dst.ip == CELL_IP and p.src.port in WEBSITE_HOST_PORT))
           and p.flags is not None and len(set(p.flags) & set(TCP_HANDSHAKE)) > 0)
...
# 3. Find all (src,dst) pair packets that are SYN packets and count them
...
# Let's look for IPs with a lot of ports in the destination
...
results = [(k,sorted([int(t) for t in list(v) if t is not None])) for k,v in ports.items()]
...
len(ordered_results[0][1])


# In[21]:


# Find duplicate packet with distinct checksum. First sort into buckets
# with same port, seq number, and options
hashed = defaultdict(list)
for p in packets:
    if p.header.protocol.isTCP():
        hashed[(p.src, p.dst, p.seq, p.header.id)].append(p)
...
# The indexes match. Find ducplicate indexes 
